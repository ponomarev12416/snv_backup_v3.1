...
def get_youngest_revision(repo_dir):
    ...
    return stdout_lines[0].decode('utf-8').strip()

######################################################################
# Main


def backup(repo_dir, backup_dir):
    # Path to repository
    ...
    try:
        youngest = get_youngest_revision(repo_dir)
    ...
    if archive_type:
        ...
        if archive_type == 'gz' or archive_type == 'bz2':
            ...
            try:
                ...
                zp = zipfile.ZipFile(
                    archive_path, 'w', zipfile.ZIP_DEFLATED, archive_type == 'zip64')
